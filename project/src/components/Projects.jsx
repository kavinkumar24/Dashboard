...
function Projects() {
  ...
  const renderGroupedDataByPltcodeAndDepartment = (
    groupedData,
    isPending = false
  ) => {
    console.log(isPending);
    const noDataMessage = isPending
      ? "No data available."
      : "No data available.";
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-3">
                {filterDepartments(departments).map(([department, qty]) => (
                  <div
                    ...
                    className={`p-4 shadow-md rounded-lg border-solid border-2 ${
                      isPending ? "border-[#eab20894]" : "border-[#22c55e93]"
                    } ${
                      theme === "light"
                  ...
                  >
                    <div
                      className={`flex justify-between p-2  rounded-md border-solid border mb-2 ${
                        theme === "light"
                    ...
                    <span className="font-normal px-2 py-9">
                      Total : <span className="font-bold">{qty}</span>
                    </span>
                  </div>
                ))}
              </div>
  ...
  const renderPendingData = () => {
    ...
    const filteredDepartments = Object.entries(departmentCounts).filter(
      ([department]) =>
        !search || department.toLowerCase().includes(search.toLowerCase())
    );
        ...
        {departmentsToShow.map((dept) =>
          groupedData[dept] ? (
            ...
            >
              ...
              <p className="text-sm">
                {isPending ? "Pending Data" : "Production Data"}:{" "}
                <span className="font-bold">{groupedData[dept]}</span>
              </p>
            </div>
          ) : null
        )}
            ...
            >
              {hasData(pendingData) && (
                <div className="mt-4 w-full">
                  ...
                  <div>
                    {renderGroupedDataByPltcodeAndDepartment(
                      groupDataByPltcodeAndDepartment(pendingData, activeTab),
                      true
                    )}
                  </div>
                </div>
