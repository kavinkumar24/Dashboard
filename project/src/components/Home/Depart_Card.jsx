...
function DepartmentDetail() {
      ...
      if (Array.isArray(dataToRender)) {
        ...
        if (type === "production") {
          ...
          filteredData = dataToRender.filter(
            (item) =>
              item.TODEPT &&
              from.includes(item.TODEPT.toUpperCase()) &&
              (search.toLowerCase() === "" ||
                item.TODEPT.toLowerCase().includes(search.toLowerCase()))
          );
  ...
  const renderFromDepartmentCards = (fromDepartments, data, label) => {
    if (!fromDepartments.length || !Object.keys(data).length) {
      return (
        <p
          className={`${theme === "light" ? "text-gray-800" : "text-red-500"}`}
        >
          No data available for the selected department's "from" departments.
        </p>
      );
    }
        ...
        {Object.entries(data).map(([dept, { quantity, pltcodes }], index) => (
          ...
          >
            ...
            >
              ...
              <p>
                Total Quantity:{" "}
                <mark className="bg-purple-200 text-black p-1">{quantity}</mark>
              </p>
            </div>
