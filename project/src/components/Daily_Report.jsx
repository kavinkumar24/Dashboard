...
function Daily_Report() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      fetch('http://localhost:8081/departmentMappings').then(response => response.json()),
  ...
  const groupedData = filteredPendingData.reduce((acc, item) => {
    ...
    if (!acc[departmentName]) {
      acc[departmentName] = { '1 to 10': 0, '11 to 15': 0, '16 to 20': 0, '21 to 25': 0, '26 to 50': 0 };
    }
      ...
      <div className={`border fixed shadow rounded-md p-4 max-w-full min-h-full inset-0 z-50 w-full md:w-[86%]  ml-0 md:ml-52 mx-auto ${theme === 'dark' ? 'bg-gray-800 border-blue-300 ' : 'bg-white border-gray-200'} sm:ml-0`}>
        ...
        <div className="flex-1 space-y-6 py-10 md:py-1">
          ...
          <div className="space-y-5 md:space-y-3">
            ...
            <p className="text-center text-lg font-semibold">No pending data available.</p>
          ) : (
            <div className="overflow-x-auto">
              <table className={` rounded-md min-w-full divide-y divide-gray-300 shadow-md ${theme==='light'?'bg-slate-400 ':'bg-slate-900'}`}>
                <thead className={`${theme === 'light' ? 'text-gray-800' : 'text-gray-300'} `}>
                  <tr>
                    <th className="px-6 py-3 text-left text-xs uppercase tracking-wider font-bold">Department</th>
                    {allRanges.map((range) => (
                      <th key={range} className="px-6 py-3 text-left text-xs font-bold uppercase tracking-wider">{range}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className={`divide-y ${theme === 'dark' ? 'bg-gray-700' : 'bg-white'}`}>
                  {Object.keys(searchedGroupedData).map((dept) => (
                    <tr key={dept} className={`border-b border-solid ${theme === 'dark' ? 'border-gray-500' : 'border-gray-300'} hover:${theme === 'light' ? 'bg-gray-100' : 'bg-gray-800'}`}>
                      <td className={`px-6 py-4 text-sm ${theme==='light'?'text-gray-700':'text-gray-400'}`}>{dept}</td>
                      {allRanges.map((range) => (
                        <td key={range} className={`px-6 py-4 text-sm ${theme==='light'?'text-gray-600':'text-gray-400'}`}>
