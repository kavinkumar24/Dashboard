...
function Detailed_task() {
  ...
  const handleWhCardClick = (wh) => {
    ...
    const sample = overAllData.filter(
      (obj) =>
        obj.TODEPT.toLowerCase() === wh.toLowerCase() &&
        obj.BRIEFNUM1.toLowerCase() === id.toLowerCase()
    );

    const whSkchData = Object.entries(
      sample.reduce((acc, curr) => {
    ...
    <>
      ...
      >
        ...
        <div className="flex-1 flex flex-col">
            ...
            Material Tailwind is a framework that enhances Tailwind CSS with additional styles and components.
                ...
                <div className="m-6 border rounded-lg border-gray-300 bg-white shadow-lg">
                  <div className="flex justify-between">
                    <h1 className="text-xl font-semibold p-2 pl-10 py-5">
                      Sketch details for{" "}
                      <span className="text-[#879FFF]">{id}</span>
                    </h1>
                    <div className="m-4">
                      <button
          ...
          <p className="ml-6 font-semibold text-xl text-gray-700">
            Pending Quantity based on{" "}
            <span className="text-[#879FFF]">Cells</span>
          </p>
          ...
          {clickedCellData.length > 0 && cell !== "UNKNOWN" && (
            <p className="ml-6 font-semibold text-xl text-gray-700">
              Warehouse Details of the cell -{" "}
              <span className="text-[#879FFF]">{cell}</span>
            </p>
          )}

          <div className="p-6 grid grid-cols-1 md:grid-cols-4 gap-4">
            {Array.isArray(clickedCellData) &&
            clickedCellData.length > 0 &&
            cell !== "UNKNOWN" ? (
              clickedCellData.map((entry, index) => (
                <div
                  ...
                  className={`shadow-md rounded-lg p-6 cursor-pointer ${
                    selectedWhCard === entry[0][0].toLowerCase()
                      ? "bg-blue-100 border-2 border-blue-400"
                      : "bg-white"
                  }`}
          ...
          {whSketchData.length > 0 && (
            <p className="ml-6 font-semibold text-xl text-gray-700">
              Sketch Details of the Warehouse{" "}
            </p>
          )}
          {whSketchData.length > 0 && (
            <div className="m-6 border rounded-lg border-gray-300 bg-white shadow-lg">
