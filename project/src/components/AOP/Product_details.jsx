...
function ProductDetailsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const [targetResponse, productionResponse, mappingsResponse] =
          await Promise.all([
  ...
  const calculateAchievedCount = (subProduct) => {
    ...
    productionData.forEach((item) => {
      if (
        item.SubCategory === subProduct &&
        productMappings.from.includes(item["From Dept"]) &&
        productMappings.to.includes(item["To Dept"])
      ) {
  ...
  return (
    ...
    >
      ...
      <div className="flex-1 flex flex-col">
        <Header theme={theme} className="p-0 m-0"  on_filter={setFilter_on}
          filter={filter_on}/>
  ...
  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
    {Object.keys(groupedProducts).length > 0 ? (
      Object.keys(groupedProducts).map((product) => (
        <div
          ...
          className={`p-4 rounded-lg shadow-md border-t-4 border-indigo-300 relative ${
            theme === "light" ? "text-gray-800 bg-white" : "text-gray-300 bg-slate-600"
          }`}
        >
          <div className={`p-2 mt-2 rounded-lg ${theme === "light" ? "bg-slate-100" : "bg-slate-500"}`}>
            ...
            <p>
              Count:{" "}
              <mark className={`rounded-md px-1 py-0.5 ${theme === "light" ? "bg-purple-200 text-black" : "bg-purple-900 text-slate-50"}`}>
                  ...
                  {Object.keys(groupedProducts[product].subProducts).map((subProduct) => (
                    <tr key={subProduct}>
                      ...
                      <td className="border px-2 py-1 text-center">
                        {Math.round(groupedProducts[product].subProducts[subProduct].total) - Math.round(groupedProducts[product].subProducts[subProduct].achieved)}
                      </td>
                    </tr>
                  ))}
                </tbody>
      ...
      <div className="text-lg text-red-500">No products found for this project.</div>
    )}
  </div>
</main>
