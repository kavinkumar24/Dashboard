...
const ProjectDetails = () => {
  ...
  const handleTop15Click = () => {
    if (!isSelected) {
      if (originalData) {
        const categoryFilteredData = originalData.filter(item => item.PROJECT === project);
  ...
  const { purityAcc, projectAcc, productAcc, subproductAcc, colorAcc } =
    aggregateData(data);
...
>
  {isSelected ? (
    ...
    "Top 15 in " + project
            ...
            {/* <button
            onClick={handleTop15Click}
            className="mb-4 w-52 px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-700"
          >
            Top 15 in {project}
          </button> */}
          <h1 className="col-span-2">Details for Zone: {project}</h1>

          {/* Purity Chart */}
          <div
            className={`order-1 col-span-2 lg:col-span-1 ${
              theme === "light" ? "bg-white" : "bg-gray-900"
            } p-4 rounded shadow-lg max-h-[400px] overflow-auto`}
          >
          ...
          <div
            className={`order-3 col-span-1 ${
              theme === "light" ? "bg-white " : "bg-gray-900"
            } p-4 rounded shadow-md overflow-auto h-[400px] custom-scrollbar`}
          >
